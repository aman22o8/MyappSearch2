...
class AppStore extends Component {
  state = {activeId: tabsList[0].tabId, initialvalue: ''}

  selectedtab = tabId => {
    // const {activeId}=this.state
    console.log(tabId)
    return this.setState({activeId: tabId})
  }

  changeingvalue = event => {
    console.log(event.target.value)
    return this.setState({initialvalue: event.target.value})
  }

  render() {
    const {activeId, initialvalue} = this.state
    const filtered = appsList.filter(each => each.category === activeId)
    const searched = filtered.filter(each =>
      each.appName.toLowerCase().includes(initialvalue.toLowerCase()),
    )
    return (
        ...
        <ul className="tabitemcontainer">
          {tabsList.map(each => (
            <TabItem
              ...
              eachitem={each}
              isActive={each.tabId === activeId}
            />
          ))}
        </ul>

        <ul className="tabitemcontainer mywidth">
          {searched.map(each => (
            <AppItem key={each.appId} eachItem={each} />
          ))}
        </ul>
      </div>
    )
